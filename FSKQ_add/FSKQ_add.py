...
sememe_em = gensim.models.KeyedVectors.load_word2vec_format(sememe_vec, binary=False) 
...
padding = tf.zeros([batch_dim, 568], tf.float32) 
sememe_feature = tf.concat([sememe_feature, padding], 1)  

output_layer = tf.add(output_layer, sememe_feature)